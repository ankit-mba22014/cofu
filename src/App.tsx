...
const CAT_COLORS: Record<string, string> = { 
  Religious: T.warm, Cultural: T.red, Social: T.success, Professional: T.info, All: T.accent 
};
  ...
  <div style={{ marginBottom: 18 }}>
    ...
    <div style={{ position: "relative" }}>
      ...
      <input type={type} placeholder={placeholder} value={value} onChange={onChange} style={{
        ...
        padding: Icon ? "14px 14px 14px 44px" : "14px 16px", color: T.text,
...
// ─── 1. DISCOVER ──────────────────────────────────────────────────────────────
const DiscoverScreen = ({ setScreen, setSelectedEvent }: any) => {
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      <div style={{ padding: "30px 24px 10px", position: "sticky", top: 0, background: T.bg, zIndex: 10 }}>
        ...
        {/* FIX 3: paddingBottom instead of pb */}
        <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 15, scrollbarWidth: "none" }} className="no-scrollbar">
      ...
      <div style={{ padding: "10px 24px 20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {rest.map((e, i) => (
            ...
            >
              <div style={{ width: 56, height: 56, borderRadius: 16, flexShrink: 0, background: `${e.color}10`, fontSize: 26, display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${e.color}20` }}>{e.emoji}</div>
...
// ─── 2. EVENT DETAIL ──────────────────────────────────────────────────────────
const EventScreen = ({ setScreen, event }: any) => {
    ...
    <div ref={screenRef} style={{ paddingBottom: 120 }}>
      ...
      <div style={{ padding: "30px 24px" }}>
        ...
        <div style={{ background: T.card, borderRadius: 20, border: `1px solid ${T.border}`, padding: 16, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button style={{ color: T.accent, fontSize: 13, fontWeight: 700, background: "none", border: "none", cursor: "pointer" }}>Follow</button>
        </div>
      </div>
...
// ─── 3. REGISTER ─────────────────────────────────────────────────────────────
const RegisterScreen = ({ event, setScreen }: any) => {
  const e = event || EVENTS[0];
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg }}>
      <div style={{ padding: "20px 24px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 16 }}>
        <button onClick={() => step === 1 ? setScreen("event") : setStep(s => s - 1)} style={{ color: T.textSub, background: "none", border: "none", cursor: "pointer" }}><ArrowLeft /></button>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: "30px 24px" }}>
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <Input label="Full Name" placeholder="e.g. Ramesh Patel" value={name} onChange={(v: any) => setName(v.target.value)} />
            <Input label="Mobile Number" placeholder="98765 43210" type="tel" value={mobile} onChange={(v: any) => setMobile(v.target.value)} icon={Smartphone} hint="Standard WhatsApp charges may apply" />
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <Input label="Custom Amount" placeholder="₹ Enter value" type="number" value={amount} onChange={(v: any) => setAmount(v.target.value)} />
  ...
  <div style={{ paddingBottom: 100 }}>
    <div style={{ padding: "30px 24px 10px" }}>
      ...
      <div style={{ background: T.successDim, borderRadius: 20, border: `1px solid ${T.successBorder}`, padding: 20, display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
        ...
        <button style={{ background: T.success, color: "#000", border: "none", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>GO LIVE</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 32 }}>
        {[
          ...
          <motion.div
            key={a.label}
            whileTap={{ scale: 0.95 }}
            style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 20, padding: 20, cursor: "pointer" }}
          >
    ...
    <div style={{ paddingBottom: 100 }}>
      <div style={{ padding: "30px 24px 10px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          ...
          <button style={{ background: T.accent, color: "#fff", width: 44, height: 44, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer" }}>
            <Plus />
          </button>
        </div>
        ...
        {tab === "pending" && (
          <div style={{ background: T.accentDim, border: `1px solid ${T.accentBorder}`, borderRadius: 16, padding: 16, display: "flex", alignItems: "center", gap: 16, marginBottom: 24 }}>
            ...
            <button style={{ background: T.accent, border: "none", width: 40, height: 40, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", cursor: "pointer" }}>
...
const RoleBanner = ({ role, setRole, setScreen }: any) => {
  ...
  const handleSwitch = () => {
    ...
    setRole(nextRole);
    if (isTreasurer) setScreen("discover");
  };
  return (
    <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 1001 }}>
      ...
      <motion.button
        ...
        style={{ background: "transparent", border: `1px solid ${T.border}`, color: T.accent, borderRadius: 8, padding: "5px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: T.fontHead, letterSpacing: "0.02em" }}
      >
        SWITCH ROLE
      </motion.button>
    </div>
...
const CreateScreen = ({ setScreen }: any) => {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ padding: "40px 24px", textAlign: "center" }}>
      ...
      <p style={{ color: T.textSub, fontSize: 15, marginBottom: 32, lineHeight: 1.6 }}>Your community page is ready. Share it on WhatsApp to start collecting registrations.</p>
      <div style={{ background: T.card, border: `1px solid ${T.accentBorder}`, borderRadius: 24, padding: 24, textAlign: "left", marginBottom: 32 }}>
        <div style={{ ...S.label, color: T.accent, marginBottom: 12, fontSize: 11 }}>EVENT LINK</div>
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: "14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 13, color: T.textSub, fontFamily: "monospace" }}>cofu.app/e/{(form.eventName||"my-event").toLowerCase().replace(/\s+/g,"-")}</span>
          <button style={{ color: T.accent, fontWeight: 700, fontSize: 13, background: "none", border: "none", cursor: "pointer" }}>COPY</button>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 40 }}>
        <button style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: "20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer" }}>
          <MessageCircle color={T.success} /><span style={{ fontSize: 12, fontWeight: 700 }}>WhatsApp</span>
        </button>
        <button style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 16, padding: "20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer" }}>
          <Share2 color={T.info} /><span style={{ fontSize: 12, fontWeight: 700 }}>Other Apps</span>
        </button>
      </div>
    ...
    <div style={{ paddingBottom: 100 }}>
      <div style={{ padding: "24px 24px 10px", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, background: T.bg, zIndex: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <button onClick={() => setScreen("discover")} style={{ color: T.textSub, background: "none", border: "none", cursor: "pointer" }}><ArrowLeft /></button>
          <div style={{ ...S.h2, fontSize: 16 }}>Host New Event</div>
          <div style={{ width: 24 }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {steps.map((s, i) => (
            <div key={s} style={{ flex: 1, height: 4, borderRadius: 2, background: step > i + 1 ? T.success : step === i + 1 ? T.accent : T.border, transition: "0.3s" }} />
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {["Mandali", "Temple", "RWA", "Club"].map(t => (
                    <button key={t} onClick={() => updateForm("orgType", t)} style={{ padding: "12px", borderRadius: 12, border: `1px solid ${form.orgType === t ? T.accent : T.border}`, background: form.orgType === t ? `${T.accent}10` : T.surface, color: form.orgType === t ? T.accent : T.text, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{t}</button>
                  ))}
                </div>
              </div>
              <Divider margin="24px 0" />
              ...
              <Input label="Email Address" placeholder="you@gmail.com" type="email" value={form.contactEmail} onChange={(e: any) => updateForm("contactEmail", e.target.value)} />
              <div style={{ marginTop: 32 }}><Btn full onClick={() => setStep(2)}>Next Step <ChevronRight size={18} /></Btn></div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <textarea placeholder="Tell people about the event..." value={form.desc} onChange={(e) => updateForm("desc", e.target.value)} style={{ width: "100%", background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: "14px", color: T.text, fontSize: 15, minHeight: 100, fontFamily: T.fontBody, resize: "none", boxSizing: "border-box" }} />
              </div>
              <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
                <Btn secondary full onClick={() => setStep(1)}>Back</Btn>
                <Btn full onClick={() => setStep(3)}>Next Step <ChevronRight size={18} /></Btn>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {["Free", "Donation", "Fixed", "Tiered"].map(t => (
                    <button key={t} onClick={() => updateForm("passType", t)} style={{ padding: "12px", borderRadius: 12, border: `1px solid ${form.passType === t ? T.accent : T.border}`, background: form.passType === t ? `${T.accent}10` : T.surface, color: form.passType === t ? T.accent : T.text, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>{t}</button>
                  ))}
                </div>
              </div>
              {form.passType === "Fixed" && (
              ...
              <div style={{ padding: "16px", background: T.card, borderRadius: 20, border: `1px solid ${T.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  ...
                  <button onClick={() => updateForm("is80G", !form.is80G)} style={{ width: 44, height: 24, borderRadius: 12, background: form.is80G ? T.accent : T.border, position: "relative", border: "none", cursor: "pointer", transition: "background 0.2s" }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#fff", position: "absolute", left: form.is80G ? 23 : 3, top: 3, transition: "left 0.2s" }} />
                  </button>
                </div>
                {form.is80G && (
                ...
                <div style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.5 }}>When enabled, donors receive a tax-compliant receipt automatically.</div>
              </div>
              <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
                <Btn secondary full onClick={() => setStep(2)}>Back</Btn>
                <Btn full onClick={() => setStep(4)}>Preview <ChevronRight size={18} /></Btn>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ padding: 20, background: T.card, borderRadius: 20, border: `1px solid ${T.border}` }}>
                  ...
                  {form.is80G && <div style={{ fontSize: 12, color: T.success, marginTop: 4 }}>✓ 80G Enabled · {form.reg80G}</div>}
                </div>
              </div>
              <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
                <Btn secondary full onClick={() => setStep(3)}>Edit</Btn>
...
// ─── NAV ──────────────────────────────────────────────────────────────────────
const NAV_ITEMS = [
  { id: "discover", icon: Search, label: "Explore", publicOnly: true },
  { id: "event", icon: Calendar, label: "Events", publicOnly: true },
  { id: "create", icon: Plus, label: "Host", publicOnly: true },
  { id: "dashboard", icon: TrendingUp, label: "Treasury", publicOnly: false },
  { id: "members", icon: Users, label: "Members", publicOnly: false },
];
...
export default function App() {
  ...
  const availableNavItems = useMemo(() =>
    NAV_ITEMS.filter(item => role === "treasurer" || item.publicOnly),
    [role]
  );
      ...
      <div style={{
        ...
        {availableNavItems.map((item) => {
          ...
          return (
            ...
            >
              {isHost ? (
                <div style={{ background: T.accent, color: "#fff", width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 2, boxShadow: `0 4px 12px ${T.accent}30` }}>
                  <item.icon size={20} />
                </div>
              ) : (
